...
inline Eigen::VectorXd computeGeneralizedAcceleration (const Eigen::VectorXd& gc, const Eigen::VectorXd& gv, const Eigen::VectorXd& gf) {
    ...
    trunk->jointTwist << trunk->jointLinearVelocity, trunk->jointAngularVelocity;
    ...
    tree_FR.trunk_reached = false;
    tree_FR.i = 0;
    tree_FR.computeJointLinearVelocity(FR_hip, counter);
    counter = 0;
    tree_FR.trunk_reached = false;
    tree_FR.i = 0;
    ...
    tree_FR.trunk_reached = false;
    tree_FR.i = 0;
    tree_FR.computeJointLinearVelocity(FR_calf, counter);
    counter = 0;
    tree_FR.trunk_reached = false;
    tree_FR.i = 0;
    tree_FR.computeJointLinearVelocity(FR_foot, counter);
    tree_FR.getCompositeBody(FR_calf, FR_foot);
    tree_FR.getX_BP(FR_calf);
    FR_calf->articulatedMassMatrix = tree_FR.getSpatialInertiaMatrix(FR_calf);
    FR_calf->articulatedBias = tree_FR.getBias(FR_calf);
    tree_FR.computeArticulatedBody(FR_calf);
    trunkArticulatedMassMatrix += trunk->articulatedMassMatrix - tree_FR.getSpatialInertiaMatrix(trunk);
    trunkArticulatedBias += trunk->articulatedBias - tree_FR.getBias(trunk);
    ...
    tree_FL.trunk_reached = false;
    tree_FL.i = 0;
    tree_FL.computeJointLinearVelocity(FL_hip, counter);
    counter = 0;
    tree_FL.trunk_reached = false;
    tree_FL.i = 0;
    tree_FL.computeJointLinearVelocity(FL_thigh, counter);
    counter = 0;
    tree_FL.trunk_reached = false;
    tree_FL.i = 0;
    tree_FL.computeJointLinearVelocity(FL_calf, counter);
    counter = 0;
    tree_FL.trunk_reached = false;
    tree_FL.i = 0;
    tree_FL.computeJointLinearVelocity(FL_foot, counter);
    tree_FL.getCompositeBody(FL_calf, FL_foot);
    tree_FL.getX_BP(FL_calf);
    FL_calf->articulatedMassMatrix = tree_FL.getSpatialInertiaMatrix(FL_calf);
    FL_calf->articulatedBias = tree_FL.getBias(FL_calf);
    tree_FL.computeArticulatedBody(FL_calf);
    trunkArticulatedMassMatrix += trunk->articulatedMassMatrix - tree_FL.getSpatialInertiaMatrix(trunk);
    trunkArticulatedBias += trunk->articulatedBias - tree_FL.getBias(trunk);
    ...
    tree_RR.trunk_reached = false;
    tree_RR.i = 0;
    tree_RR.computeJointLinearVelocity(RR_hip, counter);
    counter = 0;
    tree_RR.trunk_reached = false;
    tree_RR.i = 0;
    tree_RR.computeJointLinearVelocity(RR_thigh, counter);
    counter = 0;
    tree_RR.trunk_reached = false;
    tree_RR.i = 0;
    tree_RR.computeJointLinearVelocity(RR_calf, counter);
    counter = 0;
    tree_RR.trunk_reached = false;
    tree_RR.i = 0;
    tree_RR.computeJointLinearVelocity(RR_foot, counter);
    tree_RR.getCompositeBody(RR_calf, RR_foot);
    tree_RR.getX_BP(RR_calf);
    RR_calf->articulatedMassMatrix = tree_RR.getSpatialInertiaMatrix(RR_calf);
    RR_calf->articulatedBias = tree_RR.getBias(RR_calf);
    tree_RR.computeArticulatedBody(RR_calf);
    trunkArticulatedMassMatrix += trunk->articulatedMassMatrix - tree_RR.getSpatialInertiaMatrix(trunk);
    trunkArticulatedBias += trunk->articulatedBias - tree_RR.getBias(trunk);
    ...
    tree_RL.trunk_reached = false;
    tree_RL.i = 0;
    tree_RL.computeJointLinearVelocity(RL_hip, counter);
    counter = 0;
    tree_RL.trunk_reached = false;
    tree_RL.i = 0;
    tree_RL.computeJointLinearVelocity(RL_thigh, counter);
    counter = 0;
    tree_RL.trunk_reached = false;
    tree_RL.i = 0;
    tree_RL.computeJointLinearVelocity(RL_calf, counter);
    counter = 0;
    tree_RL.trunk_reached = false;
    tree_RL.i = 0;
    tree_RL.computeJointLinearVelocity(RL_foot, counter);
    tree_RL.getCompositeBody(RL_calf, RL_foot);
    tree_RL.getX_BP(RL_calf);
    RL_calf->articulatedMassMatrix = tree_RL.getSpatialInertiaMatrix(RL_calf);
    RL_calf->articulatedBias = tree_RL.getBias(RL_calf);
    tree_RL.computeArticulatedBody(RL_calf);
    trunkArticulatedMassMatrix += trunk->articulatedMassMatrix - tree_RL.getSpatialInertiaMatrix(trunk);
    trunkArticulatedBias += trunk->articulatedBias - tree_RL.getBias(trunk);
    ...
    trunk->articulatedMassMatrix = trunkArticulatedMassMatrix + tree_imu.getSpatialInertiaMatrix(trunk);
    trunk->articulatedBias = trunkArticulatedBias + tree_imu.getBias(trunk);

//    std::cout << trunk->articulatedMassMatrix << std::endl;
//    std::cout << trunk->articulatedBias.transpose() << std::endl;

    Eigen::VectorXd trunk_genAcceleration, gravity;
    Eigen::VectorXd generalizedAccelerations, trueGA;
    gravity.setZero(6,1);
    gravity << 0,0,9.81,0,0,0;

    trunk_genAcceleration = (Eigen::MatrixXd::Identity(6,6).transpose()*trunk->articulatedMassMatrix*Eigen::MatrixXd::Identity(6,6)).inverse()*
            (gf.head(6) - Eigen::MatrixXd::Identity(6,6).transpose()*trunk->articulatedBias);
    trunk->jointTwist_dot = Eigen::MatrixXd::Identity(6,6)*(trunk_genAcceleration);

    trunk_genAcceleration -= gravity;

    // Now re-attach legs one by one
    trunk->children.push_back(FR_hip);
    tree_FR.getGeneralizedAcceleration(trunk);
    trunk->children.pop_back();

    trunk->children.push_back(FL_hip);
    tree_FL.getGeneralizedAcceleration(trunk);
    trunk->children.pop_back();

    trunk->children.push_back(RR_hip);
    tree_RR.getGeneralizedAcceleration(trunk);
    trunk->children.pop_back();

    trunk->children.push_back(RL_hip);
    tree_RL.getGeneralizedAcceleration(trunk);
    trunk->children.pop_back();

    generalizedAccelerations.setZero(18);

    generalizedAccelerations << trunk_genAcceleration,
            FR_hip->generalizedAcceleration, FR_thigh->generalizedAcceleration, FR_calf->generalizedAcceleration,
            FL_hip->generalizedAcceleration, FL_thigh->generalizedAcceleration, FL_calf->generalizedAcceleration,
            RR_hip->generalizedAcceleration, RR_thigh->generalizedAcceleration, RR_calf->generalizedAcceleration,
            RL_hip->generalizedAcceleration, RL_thigh->generalizedAcceleration, RL_calf->generalizedAcceleration;
